    ...
    for (const item of items as Array<Record<string, unknown>>) {
      ...
      if (combos && Array.isArray(combos.combos)) {
        for (const c of combos.combos as Array<Record<string, unknown>>) {
          const rawName = String(c.groupName ?? '').trim();
          ...
          parts.push(bold(true), line(groupName), bold(false));
          for (const x of cItems) {
            parts.push(bold(true), line(`  \u21B3 ${x.name}`), bold(false));
            const itemExtras = Array.isArray(x.extras) ? (x.extras as Array<Record<string, unknown>>).filter((e) => Number(e.qty) > 0) : [];
