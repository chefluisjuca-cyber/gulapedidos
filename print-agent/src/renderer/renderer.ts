...
$('btn-connect').addEventListener('click', async () => {
  ...
  if (!token) {
    setError('Cole a chave de API primeiro.');
    return;
  }

  console.log('[Print Agent] Iniciando conexão com token:', token.slice(0, 8) + '...');
  ...
  try {
    console.log('[Print Agent] Chamando agentAPI.connect...');
    const result = await window.agentAPI.connect(token);
    console.log('[Print Agent] Resultado da conexão:', result);

    if (result.ok) {
      console.log('[Print Agent] Conexão bem-sucedida, carregando impressoras...');
      showStep('configure');
      await loadPrintersAndConfig();
    } else {
      const errMsg = result.error ?? 'Falha ao conectar. Verifique a chave de API.';
      console.error('[Print Agent] Falha na conexão:', errMsg);
      setError(errMsg);
      btn.disabled = false;
      btn.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg> Conectar';
    }
  } catch (err) {
    const errMsg = err instanceof Error ? err.message : 'Erro inesperado ao conectar.';
    console.error('[Print Agent] Erro inesperado:', err);
