...
function SignupForm({ initialPlan, onBack, onGoLogin, onDone }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      console.log('restaurant-signup response:', { status: res.status, ok: res.ok, data });
      if (!res.ok || data.error) {
        const errMsg = data.error ?? 'Erro ao cadastrar. Tente novamente.';
        console.error('restaurant-signup: cadastro falhou', { status: res.status, response: data });
        if (data.wapi_error) {
          console.error('restaurant-signup: W-API error detalhado:', data.wapi_error);
        }
        setError(errMsg);
        setLoading(false);
        return;
      ...
      if (data.wapi_error) {
        console.warn('restaurant-signup: conta criada mas WhatsApp falhou:', data.wapi_error);
      }
      onDone({ slug: data.restaurant.slug, name: data.restaurant.name });
    } catch (err) {
      console.error('restaurant-signup: erro de conexão', err);
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-6">
          ...
          <p className="text-amber-600 text-xs mt-2 font-medium">
            Plano selecionado: {plan === 'essencial' ? 'Essencial' : plan === 'pedidos_fidelidade' ? 'Profissional' : plan === 'pedidos_fidelidade_etiquetas' ? 'Premium' : plan === 'gula_etiquetas_standalone' ? 'Gula Etiquetas' : plan === 'gula_fila_standalone' ? 'Gula Fila' : plan === 'gula_feedback_standalone' ? 'Gula Feedback' : 'Essencial'}
          </p>
        </div>
