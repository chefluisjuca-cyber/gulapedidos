        ...
        {items.length === 0 ? (
          ...
          <div key={item.cartId} className="bg-slate-800/60 rounded-xl p-3 border border-slate-700/50">
            ...
            {item.comboSelections.map(cs => {
              ...
              return (
                <p key={cs.groupId} className="text-xs text-slate-500 mt-0.5">
                  {cs.groupName}: {chosen.map(i => i.name).join(', ')}
                </p>
              );
            })}
...
export default function CashierOrderDrawer({ onClose, onOrderPlaced }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [settRes, catRes, prodRes] = await Promise.all([
        ...
        supabase.from('products').select('*, combo_groups(*, combo_group_items(*)), product_extras(*)').eq('restaurant_id', restaurantId).eq('active', true).order('sort_order').order('name'),
      ]);
  ...
  async function checkout() {
    ...
    const items = cart.map(ci => ({
      ...
      customizations: {
        combos: ci.comboSelections.map(cs => ({ groupName: cs.groupName, items: cs.items.filter(i => i.qty > 0) })),
                  ...
                  {cart.length === 0 ? (
                    ...
                    <div key={item.cartId} className="bg-slate-800/60 rounded-xl p-3 border border-slate-700/50">
                      ...
                      {item.comboSelections.map(cs => {
                        const chosen = cs.items.filter(i => i.qty > 0);
                        if (!chosen.length) return null;
                        return <p key={cs.groupId} className="text-xs text-slate-500 mt-0.5">{cs.groupName}: {chosen.map(i => i.name).join(', ')}</p>;
                      })}
                      {item.extraSelections.filter(e => e.qty > 0).map(ex => (
                        <p key={ex.extraId} className="text-xs text-slate-500 mt-0.5">+{ex.qty}× {ex.name}</p>
                      ))}
