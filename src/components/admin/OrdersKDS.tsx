...
import { dispatchPrintJob, fetchPrintSettings, fetchPrinters, isAgentOnline } from '../../lib/print-agent-client';
...
export default function OrdersKDS() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (ordersRes.data) {
      setOrders(() => {
        const newOrders = ordersRes.data as Order[];
  ...
  async function handlePrintKitchen(order: Order) {
    if (restaurantId) { try { await autoDispatchToAgent(restaurantId, order, 'kitchen'); } catch { /* fall through */ } }
  ...
  async function handlePrintReceipt(order: Order) {
    if (restaurantId) { try { await autoDispatchToAgent(restaurantId, order, 'receipt'); } catch { /* fall through */ } }
                  ...
                  <div key={item.id} className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      {item.customizations?.combos?.map((c, i) => (
                        <p key={i} className="text-slate-500 text-xs ml-4">↳ {c.groupName}: {c.items.filter(x => x.qty > 0).map(x => x.name).join(', ')}</p>
                      ))}
                      {item.customizations?.extras?.filter(e => e.qty > 0).map((e, i) => (
                        <p key={i} className="text-slate-500 text-xs ml-4">↳ +{e.qty}× {e.name}</p>
                      ))}
                    </div>
