    ...
    <div className="p-3 sm:p-6 max-w-6xl mx-auto space-y-6 pb-16">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <TutorialHelpButton videoId="hiIZb8QV-LA" title="Gestão de Etiquetas" />
        </div>
      </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-wide transition-all ${
              showConfig
                ? 'bg-amber-400 text-[#0a1830] shadow-lg shadow-amber-400/30 border border-amber-300'
                : 'bg-transparent text-amber-400 border-2 border-amber-400 hover:bg-amber-400/10'
            }`}
          >
...
function ValidadeConfigPanel({ restaurantId }: { restaurantId: string | null }) {
  ...
  useEffect(() => {
    loadAlertConfig(restaurantId).then(cfg => {
      ...
      setWhatsapp(cfg.responsavelWhatsapp);
  ...
  async function handleSave() {
    ...
    try {
      ...
      await saveAlertConfig(restaurantId, {
        ...
        responsavelWhatsapp: cleanWhatsapp,
  ...
  return (
    <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-5 space-y-4">
      ...
      <p className="text-xs text-slate-400 bg-[#0f2040] rounded-xl px-3 py-2.5 border border-[#1e3868]">
        Atribua um responsável para que receba notificações via WhatsApp de produtos a vencer.
      </p>
      ...
      <div className="space-y-1.5">
        ...
        <input
          ...
          placeholder="Ex: 11964233303"
          className="w-full bg-[#0a1830] border border-emerald-500/30 rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:border-emerald-500 focus:outline-none transition-colors"
        />
        <p className="text-[11px] text-slate-500">Digite apenas números. O prefixo 55 será adicionado automaticamente. Ex: 11964233303</p>
      </div>
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        {ativas && (
          <span className="text-xs text-emerald-400 flex items-center gap-1">
            ...
            Alertas ativos{horario ? ` às ${horario}` : ''}{whatsapp ? ' via WhatsApp' : ''}
          </span>
        )}
      </div>
    </div>
