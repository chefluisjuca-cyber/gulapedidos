...
import {
  ...
  ArrowUpRight, Check, Bot, Clock, MessageSquare, Gift,
} from 'lucide-react';
...
export default function LandingPage() {
  ...
  useEffect(() => {
    supabase
      ...
      .maybeSingle()
      .then(({ data }) => {
        setShowAssistant(data?.show_virtual_assistant ?? false);
      });
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 py-6 bg-white">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-6 py-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-slate-900">Dúvidas? Acesse nosso vídeo tutorial completo!</p>
              <p className="text-sm text-slate-600 mt-0.5">Aprenda a configurar e usar o sistema passo a passo.</p>
            </div>
          </div>
          <a
            href="https://www.youtube.com/@GulaDigital"
            target="_blank"
      ...
      <section id="planos" className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24 bg-white scroll-mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 items-stretch">
          {PRICING_CARDS.map((card) => {
            ...
            const cycleLabel = cycle === 'mensal' ? '/mês' : cycle === 'semestral' ? '/semestre' : '/ano';
            return (
            <div
              key={card.planId}
              ...
              <div className="mb-5">
                ...
                {cycle !== 'mensal' && (
                  <span className="text-xs text-emerald-600 font-medium">
                    ≈ R$ {fmtBRL(displayPrice / (cycle === 'semestral' ? 6 : 12))}/mês
                  </span>
                )}
              </div>
            ...
            <div className="relative flex flex-col rounded-3xl border-2 border-emerald-400 bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-6 sm:p-8 shadow-xl shadow-emerald-200/50">
              ...
              <div className="pt-5 border-t border-emerald-100">
                {(() => {
                  ...
                  const etiqCycleLabel = standaloneCycle === 'mensal' ? '/mês' : standaloneCycle === 'semestral' ? '/semestre' : '/ano';
                    ...
                    <>
                      <div className="mb-4">
                        <div className="flex items-baseline gap-1">
                          ...
                          <span className="text-sm text-slate-500">{etiqCycleLabel}</span>
                        </div>
                        {standaloneCycle !== 'mensal' && (
                          <p className="text-xs text-emerald-600 font-medium">
                            ≈ R$ {fmtBRL(etiqPrice / (standaloneCycle === 'semestral' ? 6 : 12))}/mês
                          </p>
                        )}
            ...
            <div className="relative flex flex-col rounded-3xl border-2 border-blue-400 bg-gradient-to-br from-blue-50 via-white to-sky-50 p-6 sm:p-8 shadow-xl shadow-blue-200/50">
              ...
              <div className="pt-5 border-t border-blue-100">
                {(() => {
                  ...
                  const filaCycleLabel = standaloneCycle === 'mensal' ? '/mês' : standaloneCycle === 'semestral' ? '/semestre' : '/ano';
                  return (
                    <>
                      <div className="mb-4">
                        <div className="flex items-baseline gap-1">
                          <span className="text-3xl font-extrabold text-blue-700">R$ {fmtBRL(filaPrice)}</span>
                          <span className="text-sm text-slate-500">{filaCycleLabel}</span>
                        </div>
                        {standaloneCycle !== 'mensal' && (
                          <p className="text-xs text-blue-600 font-medium">
                            ≈ R$ {fmtBRL(filaPrice / (standaloneCycle === 'semestral' ? 6 : 12))}/mês
                          </p>
                        )}
                        <span className="text-xs text-blue-600 font-medium block mt-1">7 dias grátis para testar</span>
                      </div>
                      <a
                        href="https://www.gulafila.com.br"
                        target="_blank"
                        rel="noopener noreferrer"
                      ...
                      <p className="mt-3 text-xs text-slate-400 text-center">Produto independente · Site externo</p>
                    </>
                  );
                })()}
              </div>
            ...
            <div className="relative flex flex-col rounded-3xl border-2 border-amber-400 bg-gradient-to-br from-amber-50 via-white to-orange-50 p-6 sm:p-8 shadow-xl shadow-amber-200/50">
              ...
              <div className="pt-5 border-t border-amber-100">
                {(() => {
                  ...
                  const fbPrice = fbEntry?.price ?? STANDALONE_FEEDBACK_CARD.priceNew;
                  const fbCycleLabel = standaloneCycle === 'mensal' ? '/mês' : standaloneCycle === 'semestral' ? '/semestre' : '/ano';
                  const fbPriceId = fbEntry?.stripe_price_id;
                  return (
                    <>
                      <div className="mb-4">
                        <div className="flex items-baseline gap-1">
                          <span className="text-3xl font-extrabold text-amber-700">R$ {fmtBRL(fbPrice)}</span>
                          <span className="text-sm text-slate-500">{fbCycleLabel}</span>
                        </div>
                        {standaloneCycle !== 'mensal' && (
                          <p className="text-xs text-amber-600 font-medium">
                            ≈ R$ {fmtBRL(fbPrice / (standaloneCycle === 'semestral' ? 6 : 12))}/mês
                          </p>
                        )}
      ...
      {showAssistant && <GulaEspecialistaWidget />}
    </div>
  );
}
