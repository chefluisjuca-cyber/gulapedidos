...
export default function ProductDrawer({ product, onClose, inline = false, onAdd }: Props) {
  ...
  function adjustComboItem(groupId: string, itemId: string, delta: number) {
    ...
    setComboSelections(prev => prev.map(cs => {
      ...
      if (isRadio) {
        return { ...cs, items: cs.items.map(item => ({ ...item, qty: delta > 0 ? (item.id === itemId ? 1 : 0) : 0 })) };
      }
      return {
        ...cs,
        items: cs.items.map(item => {
          ...
          if (delta > 0 && currentTotal >= group.max_qty) return item;
  ...
  function computeRegularUnitTotal() {
    ...
    comboSelections.forEach(cs => cs.items.forEach(i => {
      ...
      if (i.qty > 0) total += comboItemExtraTotal(i.id) * i.qty;
    }));
                  ...
                  {visibleItems.map(item => {
                    ...
                    return (
                      <div key={item.id}>
                        {isRadio ? (
                          <button
                            onClick={() => adjustComboItem(cs.groupId, item.id, selected ? -1 : 1)}
                          ...
                          <div className={`flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-all ${
                            selected ? 'border-amber-500 bg-amber-500/10' : 'border-slate-700 bg-slate-800/60'
                          }`}>
                            <div className="flex-1 min-w-0">
                              <p className={`text-sm font-medium ${selected ? 'text-amber-300' : 'text-slate-200'}`}>{item.name}</p>
                              {item.priceDelta > 0 && (
                                <p className="text-xs text-amber-500 mt-0.5">+R$ {item.priceDelta.toFixed(2).replace('.', ',')}{item.qty > 1 ? ' / un.' : ''}</p>
                              )}
                              {item.observations && (
                                <p className="text-xs text-slate-500 italic mt-0.5">{item.observations}</p>
                              )}
                            ...
                            <div className="flex items-center gap-2 shrink-0 ml-3">
                              <button
                                onClick={() => adjustComboItem(cs.groupId, item.id, -1)}
                                disabled={item.qty === 0}
                                className="w-7 h-7 rounded-full border-2 border-slate-600 flex items-center justify-center text-slate-300 disabled:opacity-30 hover:border-amber-500 hover:text-amber-400 transition-colors"
                              >
                              ...
                              <button
                                onClick={() => adjustComboItem(cs.groupId, item.id, 1)}
                                disabled={groupTotal >= group.max_qty}
                                className="w-7 h-7 rounded-full border-2 border-slate-600 flex items-center justify-center text-slate-300 disabled:opacity-30 hover:border-amber-500 hover:text-amber-400 transition-colors"
                              >
                        ...
                        {selected && itemExtras.length > 0 && (
                          <div className="ml-4 mt-1.5 pl-2 border-l-2 border-amber-500/30 space-y-1.5">
                            <p className="text-[11px] text-amber-400/80 font-medium">Adicionais para {item.name}{item.qty > 1 ? ` (${item.qty}x — aplicado a cada unidade)` : ''}:</p>
                            {itemExtras.map(ex => (
                              <div key={ex.id} className="flex items-center justify-between py-1">
                                <div>
                                  <p className="text-xs text-slate-200">{ex.name}</p>
                                  <p className="text-[11px] text-amber-400 font-medium">+R$ {ex.price.toFixed(2).replace('.', ',')}{item.qty > 1 ? ` x${item.qty} = +R$ ${(ex.price * ex.qty * item.qty).toFixed(2).replace('.', ',')}` : ''}</p>
                                </div>
                                <div className="flex items-center gap-2">
                                  <button
