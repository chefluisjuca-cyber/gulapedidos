        ...
        {(order.order_items ?? []).map(item => (
          <div key={item.id}>
            ...
            {item.customizations?.combos?.map((c, i) => {
              ...
              return (
                <div key={i} className="ml-7 mt-0.5">
                  {gn && <p className="text-slate-400 text-xs">{gn}</p>}
                  {chosen.map((x, xi) => (
                    <div key={xi}>
                      <p className="text-slate-200 text-xs font-medium ml-2">{x.qty > 1 ? `${x.qty}x ` : ''}{x.name}</p>
                      {(x.extras ?? []).filter(e => e.qty > 0).map((e, ei) => (
                        <p key={ei} className="text-slate-400 text-xs ml-4">+{e.qty}× {e.name}</p>
                      ))}
                    </div>
                  ))}
                </div>
...
export default function KitchenKDS() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setOrders(incoming.map(o => ({ ...o, order_items: normalizeOrderItems(o.order_items ?? []) })));
  }, [triggerAlert]);
  ...
  return (
    <div className="min-h-screen print:min-h-0 bg-[#080d14] text-white flex flex-col">
